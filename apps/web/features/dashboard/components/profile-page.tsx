...
const ROLE_META: Record<OrganizationRole, {
  label: string;
...
const TIER_BADGE: Record<string, string> = {
  elevated:    "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300",
  approver:    "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300",
...
const THRESHOLDS = [
  { label: "Under $500",       range: "< $500",        roles: [] as OrganizationRole[],         description: "Auto-approved" },
  { label: "$500 – $4,999",    range: "$500–$4,999",   roles: ["APPROVER"] as OrganizationRole[], description: "Requires Approver" },
  { label: "$5,000 and above", range: "$5,000+",       roles: ["APPROVER", "FINANCE_MANAGER"] as OrganizationRole[], description: "Requires Approver + Finance Manager" },
];

const ELEVATED: OrganizationRole[] = ["OWNER", "ADMIN", "PROCUREMENT_MANAGER"];

function canHandleTier(role: OrganizationRole, tierRoles: OrganizationRole[]): boolean {
...
export function ProfilePage() {
  ...
  const canSeeAllDecisions = ELEVATED.includes(role) || role === "APPROVER" || role === "FINANCE_MANAGER";
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <section className="bg-card border border-border rounded-xl p-6 space-y-4">
        <h3 className="text-sm font-semibold text-foreground">Organization & Role</h3>

        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-foreground font-medium">{activeOrg?.name}</p>
            <p className="text-xs text-muted-foreground capitalize mt-0.5">
              {activeOrg?.type?.toLowerCase()} organization
            </p>
          </div>
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${TIER_BADGE[meta.tier]}`}>
            {meta.label}
          </span>
        </div>
      ...
      <section className="bg-card border border-border rounded-xl p-6 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">Approval Authority</h3>
        </div>
        ...
        <div className="space-y-2">
          {THRESHOLDS.map((tier) => {
            ...
            return (
              ...
              >
                <div>
                  <p className={`text-sm font-medium ${can ? "text-emerald-700 dark:text-emerald-300" : "text-muted-foreground"}`}>
                    {tier.label}
                  </p>
      ...
      {myRequests.length > 0 && (
        <section className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
            ...
            <Link href={`/${locale}/dashboard/requisitions`} className="text-xs text-muted-foreground hover:text-foreground transition-colors">
              View all →
            </Link>
          </div>
            ...
            {myRequests.slice(0, 5).map((r) => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-sm text-foreground truncate max-w-[260px]">{r.title}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{format(new Date(r.createdAt), "MMM d, yyyy")}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0 ml-3">
                  <span className="text-sm tabular-nums text-foreground">
                    {new Intl.NumberFormat(undefined, { style: "currency", currency: r.currency }).format(r.totalAmount)}
                  </span>
      ...
      {canSeeAllDecisions && myDecisions.length > 0 && (
        <section className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
            <h3 className="text-sm font-semibold text-foreground">Decisions Made</h3>
            <span className="text-xs text-muted-foreground">{myDecisions.length} total</span>
          </div>
          <div className="divide-y divide-border">
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {r.approvedAt ? format(new Date(r.approvedAt), "MMM d, yyyy") : "—"}
                  </p>
                </div>
                <div className="flex items-center gap-3 shrink-0 ml-3">
                  <span className="text-sm tabular-nums text-foreground">
      ...
      {ELEVATED.includes(role) && (
        <section className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="space-y-2">
            {[
              { label: "Budget limits & policy configuration", desc: "Set org-wide spending budgets per period" },
              { label: "Full audit log", desc: "Complete history of all approval decisions across the org" },
              { label: "Member management", desc: "Invite, remove, and change roles for org members" },
            ].map(({ label, desc }) => (
              <div key={label} className="flex items-start gap-3 px-4 py-3 rounded-lg bg-secondary/30 border border-border">
